 ...
 */
				...
				if (objData != null && objData.size() > 0) {
					...
					for (int i = 0; i < objData.size(); i++) {
						...
						for (Object o : map.keySet()) {
							...
							ws.addCell(new Label(j, i + 1,str ,wcf));
							j++;
						}
					}
				} else {
	 ...
	 *            列名称集合,如：{物品名称，数量，单价}
	 */
	public static void exportexcel(HttpServletResponse response, String filename,List<Map<String, Object>> listData, List<String> columns) {
		// 调用上面的方法、生成Excel文件
		response.setContentType("application/vnd.ms-excel");
		response.setHeader("Content-Disposition","attachment;filename="+filename);
		try {
			response.setHeader("Content-Disposition", "attachment;filename="+ new String(filename.getBytes("gb2312"), "ISO8859-1") + ".xls");
			exportToExcel(response, listData, "sheet1", columns);
